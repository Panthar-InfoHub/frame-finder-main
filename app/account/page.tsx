...
export default async function ProfilePage() {
  const [userResult, orderResult, wishlistResult] = await Promise.all([
    getUser(),
    getOrderByUser(),
    getWishlist(),
  ]);
  ...
  // Process wishlist images
  const wishlist_items =
    wishlistResult.success && wishlistResult.data?.items
      ? await Promise.all(
          wishlistResult.data.items.map(async (item: any) => {
            ...
            // Process item-level images (for accessories)
            if (item.images?.[0]) {
              const processedImages = await Promise.all(
                item.images.map(async (img: string) => await getImageUrl(img))
              );
              processedItem = {
                ...processedItem,
  ...
  return (
    <main className="min-h-screen">
      ...
      <div className="max-w-7xl mx-auto p-4 sm:p-6 lg:p-8">
        <Link
          href="/"
          className="flex items-center gap-2 text-gray-700 mb-2 hover:opacity-70 transition-opacity pb-8"
        >
            ...
            <div className="flex-1">
              <div className="mb-8">
                ...
                <TabsContent value="wishlist" className="mt-0">
                  <h1 className="text-3xl sm:text-4xl font-bold text-black mb-6">My Wishlist</h1>
                  <WishlistTab items={wishlist_items} />
                </TabsContent>
