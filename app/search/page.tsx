  ...
  const newArrivals = await Promise.all(
    response.data.map(async (product: any) => {
      const rawUrl: string | undefined =
        product?.variants?.[0]?.images?.[0]?.url || product?.images?.[0]?.url;
      ...
      <section className="relative h-[350px] md:h-[500px] w-full overflow-hidden bg-neutral-800">
        ...
        <div className="relative z-20 flex items-center justify-center h-full">
          <Form
            action="/search"
            scroll={false}
            className="search-form max-w-3xl w-full min-h-20 bg-white border-[5px] border-emerald-500 rounded-[80px] mt-8 px-5 flex flex-row items-center gap-5"
          >
            <InputGroup className="flex-1 font-bold placeholder:font-semibold placeholder:text-black-100 w-full h-auto">
              <InputGroupInput
                ...
                defaultValue={query || ""}
                placeholder="Search..."
                className="flex-1 font-bold placeholder:font-semibold placeholder:text-black-100 w-full h-auto  text-[24px]!"
              />
              <InputGroupAddon>
                {" "}
                <Search className="size-8 text-emerald-500 font-extrabold" />
              </InputGroupAddon>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-6 mb-8">
                {data.products.length > 0 ? (
                  data.products.map((product) => (
                    <Link
                      href={`/${getProductUrlType(product.productType)}/${product._id}?variantId=${
